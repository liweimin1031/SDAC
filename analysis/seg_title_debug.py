...
def dataPrepare():
    ...
    for obj in objs:
        ...
        title_document.extend(title)
        title_document.extend(content)
        ...
        lda_documents.append(title_document)
...
def selectDoc():
    ...
    date_query = {'$gte':start_date, '$lt':end_date}
    query={'post_create_date':date_query,'category':'金融財經投資區'}
    result_list=post.find(query,{'doc':1,'doc_seg':1,'post_create_date':1,'posts.text':1,'posts.seg':1,'title':1,'title_seg':1,'content':1,'content_seg':1}).sort([('post_create_date',1)]) # get doc
    #result_list=post.find(query,{'post_create_date':1,'content':1,'content_seg':1,'title':1,'title_seg':1}).sort([('post_create_date',1)]) # get title + content
    return result_list
...
def getDocTopic(lda_data):
    word_weight_list=lda_data['keywords']
    ...
    for i, document in enumerate( documents):
        ...
        for j, temp in enumerate(c_topic_rate_list):  #loop topic and get doc's topic rate in this topic
            #doc_seg=document['doc_seg']
            doc_seg=document['title_seg']+document['content_seg']
            topic_weight=temp
...
if __name__=='__main__':
    ...
    #by month
    lda_data=dataPrepare()
    with open('../web/view/data/title_financial_aug.json', 'w')as f1, open('../web/view/data/removed_num.json','w')as f2:
        lda_json=getDocTopic(lda_data)
