...
if ( isset($_SERVER['PATH_INFO']) ) {
    switch ($_SERVER['PATH_INFO']) {
        ...
        case '/getRelatedDocuments':
            $docs = $_REQUEST['docs'];
            $result = getRelatedDocuments($docs);
            $msg->success = true;
            $msg->data = $result;
            break;
        default:
...
function getRelatedDocuments($ids){
    $docids = array();
    $result = array();
    foreach($ids as $id){

        $cursor = MongoDao::searchOneById('discuss_news3',$id);
        $temp = new \stdClass();
